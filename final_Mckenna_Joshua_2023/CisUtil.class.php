 ...
 *
 * Modified 2023/12/12 by Joshua Mckenna
 */
class CisUtil
{
...
/*    public
    static function cleanMysqlInputs($string)
    {
        $mysqli = new mysqli();
    ...
    /**
     * Used to display the stories to the website
     * @param $id
     * @param $title
     * @param $story
     * @param $isLoggedin
     * @return void
     */
    public static function addStory($id = 0, $title = "title", $story = "A story", $isLoggedin = true)
    {
        $glyphEditIcon = "";
        echo "<div class='panel panel-default'>";
        // if logged in do this
        if  ($isLoggedin){
            $glyphEditIcon = "<span style='float:right'><a title='edit this story' href='editNews.php?id={$id}'><span class='glyphicon glyphicon-edit' aria-hidden='true'></span></a></span>";
        } else {//else
            $glyphEditIcon = "";
        }
        echo "<div class='panel-heading'>{$title}{$glyphEditIcon}</div>";
        echo "<div class='panel-body'>{$story}</div>";
        echo "</div>";
    }
